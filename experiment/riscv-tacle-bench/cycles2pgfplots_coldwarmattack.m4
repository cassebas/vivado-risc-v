...
define(fixed_cache_letter_4, ``i'')dnl
define(variable_cache_letter_4, d)dnl
dnl
...
\begin{figure}
  ...
  \begin{tikzpicture}
    \begin{axis}
      ...
      table [
        x expr = \coordindex,
        y expr = \thisrow{cycles_writeattack_cache},
      ] {data/rv32-experiment-benchmark-`'exp_nr`'-`'lookup_fixed_cache(exp_nr)`'$1`'.csv};
      \addlegendentry{{\scriptsize write attack cache}}
    \end{axis}
  \end{tikzpicture}
