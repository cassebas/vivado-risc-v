...
/*
 Here some extra classes are defined that are all based
 on the 32-bit small Rocket core. They all vary in terms
 of the instruction and data caches.
 */

/*
 rv32_i512b_d512b : small RV 32-bit core with
  - 512B i-cache direct mapped
  - 512B d-cache direct mapped
 */
class rv32_i512b_d512b extends Config(
...
/*
 rv32_i1k_d512b : small RV 32-bit core with
  - 1KiB i-cache direct mapped
  - 512B d-cache direct mapped
 */
class rv32_i1k_d512b extends Config(
  new WithNBreakpoints(8) ++
...
/*
 rv32_i512b_d1k : small RV 32-bit core with
  - 512B i-cache direct mapped
  - 1KiB d-cache direct mapped
 */
class rv32_i512b_d1k extends Config(
  new WithNBreakpoints(8) ++
  new WithL1ICacheSets(8) ++
  new WithL1ICacheWays(1) ++
...
/*
 rv32_i1k_d1k : small RV 32-bit core with
  - 1KiB i-cache direct mapped
  - 1KiB d-cache direct mapped
 */
class rv32_i1k_d1k extends Config(
  new WithNBreakpoints(8) ++
  new WithL1ICacheSets(16) ++
  new WithL1ICacheWays(1) ++
  new WithL1DCacheSets(16) ++
...
/*
 rv32_i2k_d1k : small RV 32-bit core with
  - 2KiB i-cache direct mapped
  - 1KiB d-cache direct mapped
 */
class rv32_i2k_d1k extends Config(
  new WithNBreakpoints(8) ++
...
/*
 rv32_i1k_d2k : small RV 32-bit core with
  - 1KiB i-cache direct mapped
  - 2KiB d-cache direct mapped
 */
class rv32_i1k_d2k extends Config(
  new WithNBreakpoints(8) ++
  new WithL1ICacheSets(16) ++
  new WithL1ICacheWays(1) ++
...
/*
 rv32_i2k_d2k : small RV 32-bit core with
  - 2KiB i-cache direct mapped
  - 2KiB d-cache direct mapped
 */
class rv32_i2k_d2k extends Config(
  new WithNBreakpoints(8) ++
  new WithL1ICacheSets(32) ++
  new WithL1ICacheWays(1) ++
  new WithL1DCacheSets(32) ++
...
/*
 rv32_i4k_d2k : small RV 32-bit core with
  - 4KiB i-cache direct mapped
  - 2KiB d-cache direct mapped
 */
class rv32_i4k_d2k extends Config(
  new WithNBreakpoints(8) ++
...
/*
 rv32_i2k_d4k : small RV 32-bit core with
  - 2KiB i-cache direct mapped
  - 4KiB d-cache direct mapped
 */
class rv32_i2k_d4k extends Config(
  new WithNBreakpoints(8) ++
  new WithL1ICacheSets(32) ++
  new WithL1ICacheWays(1) ++
...
/*
 rv32_i4k_d4k : small RV 32-bit core with
  - 4KiB i-cache direct mapped
  - 4KiB d-cache direct mapped
 */
class rv32_i4k_d4k extends Config(
  new WithNBreakpoints(8) ++
  new WithL1ICacheSets(64) ++
  new WithL1ICacheWays(1) ++
  new WithL1DCacheSets(64) ++
...
/*
 rv32_i8k_d4k : small RV 32-bit core with
  - 8KiB i-cache 2-way associative
  - 4KiB d-cache direct mapped
 */
class rv32_i8k_d4k extends Config(
  new WithNBreakpoints(8) ++
  new WithL1ICacheSets(64) ++
...
/*
 rv32_i4k_d8k : small RV 32-bit core with
  - 4KiB i-cache direct mapped
  - 8KiB d-cache 2-way associative
 */
class rv32_i4k_d8k extends Config(
  new WithNBreakpoints(8) ++
  new WithL1ICacheSets(64) ++
  new WithL1ICacheWays(1) ++
  new WithL1DCacheSets(64) ++
...
/*
 rv32_i8k_d8k : small RV 32-bit core with
  - 8KiB i-cache 2-way associative
  - 8KiB d-cache 2-way associative
 */
class rv32_i8k_d8k extends Config(
  new WithNBreakpoints(8) ++
  new WithL1ICacheSets(64) ++
  new WithL1ICacheWays(2) ++
  new WithL1DCacheSets(64) ++
...
/*
 rv32_i16k_d8k : small RV 32-bit core with
  - 16KiB i-cache 4-way associative
  - 8KiB d-cache 2-way associative
 */
class rv32_i16k_d8k extends Config(
  new WithNBreakpoints(8) ++
  new WithL1ICacheSets(64) ++
...
/*
 rv32_i8k_d16k : small RV 32-bit core with
  - 8KiB i-cache 2-way associative
  - 16KiB d-cache 4-way associative
 */
class rv32_i8k_d16k extends Config(
  new WithNBreakpoints(8) ++
  new WithL1ICacheSets(64) ++
  new WithL1ICacheWays(2) ++
  new WithL1DCacheSets(64) ++
...
/*
 rv32_i16k_d16k : small RV 32-bit core with
  - 16KiB i-cache 4-way associative
  - 16KiB d-cache 4-way associative
 */
class rv32_i16k_d16k extends Config(
  new WithNBreakpoints(8) ++
  new WithL1ICacheSets(64) ++
  new WithL1ICacheWays(4) ++
  new WithL1DCacheSets(64) ++
